...
如果验证集得到如下分类报告（准确率约 0.70，宏平均 F1 约 0.59）：
...
整体表现是合理的：数据集中正向样本数量明显更多，模型对正向类别的召回率更高、带动整体准确率与加权 F1；而中性类别样本较少，导致 precision/recall 偏低，宏平均 F1 被拉低。这类结果符合类别不平衡场景的常见现象，可通过增加中性样本、改进数据增强或调整损失权重进一步优化。
